...
# Base class values, organizaed by Resource Type name( according to Templates Directory of repo )
    baseParameters = {
        "resourceType" : {
            "CDNProfile": {
                "CDNProfile": {
                    "value": "cdnstaticspa{}1".format(self.getRegionStub())
                },
                "CDNLocation": {
                    "value": self.businessRegion
                },
                "Endpoints": {
                    "value": [
                        {
                            "name": "account-staticspa-{}-{}".format(getEnvironmentStub),
                            "originPath": "/account",
                            "originHost": "{}-account-staticspa.trafficmanager.net".format()
                        },
                        {
                            "name": "expert-staticspa-{{environmentStub}}-{{regionStub}}".format(),
                            "originPath": "/expert",
                            "originHost": "{{environmentStub}}-account-staticspa.trafficmanager.net".format()
                        }
                    ]
                }
            }
